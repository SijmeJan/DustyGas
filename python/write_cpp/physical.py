...
def write_physical(output_dir, solver_name, n_dust):
    ...
    for i in range(0, 4 + 4*n_dust):
        body.append('  if (!std::isfinite(localObservablesMin[{}])) return false;\n'.format(i))
        body.append('  if (!std::isfinite(localObservablesMax[{}])) return false;\n'.format(i))

    body.append('\n  // Check for positive densities\n')
    for i in range(0, n_dust + 1):
        body.append('  if (localObservablesMin[{}] <= 0.0) return false;\n'.format(4*i))
